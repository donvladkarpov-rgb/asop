...
if ! docker info >/dev/null 2>&1; then
  echo "ERROR: Docker недоступен. Запусти Docker Desktop."
  exit 1
fi

check_jars() {
  local missing=0
  for svc in crypto-service gateway-service terminal-service; do
...
$COMPOSE up -d --no-deps --build gateway-service terminal-service
info "Ожидание 30 сек..."
sleep 30
...
info ""
info "Android терминал: https://localhost:8080"
info "cert-sign: POST /api/v1/terminals/cert-sign (open HTTPS)"
info "sync endpoints: /api/v1/sync/** (mTLS)"
