...
COMMENT ON TABLE ASOP_CARD_MIFARES IS
    'MIFARE-карты системы. Поддерживает роли, VCM1/DESFire identity, аудит аутентификаций и техническую диагностику NFC.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.CARD_ROLE IS 'Primary role карты (VCM1 high-bit OR DESfire principal). Определяет права и DN-шаблон сертификата.';
...
CREATE INDEX idx_mifare_uid ON ASOP_CARD_MIFARES (UID) WHERE DELETED_AT IS NULL;

-- VCM1 CHECK (промпт 008): IDENTITY_JSON либо NULL, либо начинается с VCM1-magic
-- {"format":"VCM1"...}. Если не VCM1 — это legacy DESfire JSON.
ALTER TABLE ASOP_CARD_MIFARES
    ADD CONSTRAINT chk_identity_json_vcm1_or_old CHECK (
        IDENTITY_JSON IS NULL OR
        IDENTITY_JSON LIKE '{"format":"VCM1"%' OR
        IDENTITY_JSON NOT LIKE '{"format":%'  -- legacy без поля "format"
    );

-- Комментарий: IDENTITY_JSON переключается на VCM1 ({"format":"VCM1", cardId, bitmask, entity}):
--   - формат VCM1 для MIFARE Classic карт (промпт 008).
--   - legacy DESfire PKI формат — без поля "format" (или text-only "signedJson").
COMMENT ON COLUMN ASOP_CARD_MIFARES.IDENTITY_JSON IS
    'Identity JSON. Либо VCM1 (промпт 008: {format:"VCM1", bitmask, entity{type,id}}), либо legacy DESfire (для PKI-карт). NULL для неактивированных карт.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.IDENTITY_SIGNATURE IS
    'RSA-PSS signature для DESFire PKI карт (legacy). NULL для VCM1-карт (MIFARE Classic unsigned).';
...
DO $body$ BEGIN
    ALTER TABLE ASOP_USER_KRS ADD CONSTRAINT fk_uks_krs
        FOREIGN KEY (AUDIT_SERVICE_ID) REFERENCES ASOP_AUDIT_SERVICES (AUDIT_SERVICE_ID);
EXCEPTION WHEN duplicate_object THEN NULL;
END $body$;
