 ...
 * На карте лежит ТОЛЬКО в sector 1 (3 data-блока):
 *
 * - block 0: "VCM1" magic [0..3] + bitmask UInt16 LE [4..5] + tripsLeft UInt16 LE [10..11] + reserved
 ...
 *
 * Байты [10..11] (ранее reserved) хранят количество поездок UInt16 LE.
 * Старые VCM1-карты с нулями в [10..11] читаются как tripsLeft = 0 (обратная совместимость).
 */
data class CardIdentityVcm1(
    ...
    fun encodeAsBytes(): ByteArray {
        ...
        System.arraycopy(uuidToBytes(cardId), 0, buf, CARD_ID_OFFSET, UUID_BYTE_LEN)
    ...
    companion object {
        ...
        const val TRIPS_OFFSET = 10                  // block 0 [10..11] — tripsLeft UInt16 LE
        ...
        fun decodeFromBytes(buf: ByteArray): CardIdentityVcm1? {
            ...
            val entity = if (entityBytes.all { it == 0x00.toByte() }) null
                ...
                val type = EntityType.forAsopCardTypeOrdinal(lowestSetBitOrdinal(bitmask))
                    ?: return null
                EntityRef(type = type, id = uuidFromBytes(entityBytes, 0))
            }
