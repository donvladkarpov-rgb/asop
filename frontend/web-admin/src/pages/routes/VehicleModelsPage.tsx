...
import { getVehicleModels, createVehicleModel, updateVehicleModel, deleteVehicleModel } from '../../api/routes';
import type { VehicleModel } from '../../types/route';

export function VehicleModelsPage() {
  const qc = useQueryClient();
  const { data, isLoading, error } = useQuery({ queryKey: ['vehicle-models'], queryFn: getVehicleModels });
  const [edit, setEdit] = useState<Partial<VehicleModel> | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);

  const createMut = useMutation({
    mutationFn: createVehicleModel,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['vehicle-models'] }); setShowForm(false); setFormError(null); },
    onError: (e: any) => setFormError(e?.response?.data?.error || e?.message || 'Ошибка создания'),
  });
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<VehicleModel> }) => updateVehicleModel(id, data as any),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['vehicle-models'] }); setEdit(null); setFormError(null); },
    ...
    <div>
      ...
      {(showForm || edit) && (
        <div className="form-card">
          <h3>{edit?.id ? 'Редактировать' : 'Создать'} модель ТС</h3>
          ...
          <form onSubmit={(e) => {
            ...
            <div className="form-grid">
              <div className="form-group">
                ...
                <input name="modelName" defaultValue={(edit as any)?.modelName || ''} required />
              </div>
            </div>
