...
- **NFC:** 13.56 МГц, ISO/IEC 14443 (Type A&B), ISO 18092, Felica, Mifare — поддерживает DESFire EV1/EV2/EV3 (ISO 14443-4). **EMV-доступ ПОДТВЕРЖДЁН РАБОЧИМ (2026-09-21):** лицензионное ядро FTSDK `Emv` доступно стороннему приложению — `Emv.searchCard(2)` (cardType=2=`CARD_TYPE_RF`) детектит МИР-карту с обменом ATS; `Emv.startEMV` стартует (`startTransaction`, TransData задаётся). **PAN карта отдаёт только внутри полной EMV-транзакции** (`SELECT→GPO→READ RECORD` в `startEMV`), а НЕ через `getCardData("5A")`/`GET DATA` вне транзакции (отвечает «данные недоступны»). Сырой `NfcReader.openCardEx(0,…)` даёт `error=89` (`ERR_OP_TIMEOUT`) — там тоже нужен корректный cardType=2/RF, а не 0/IC. **Секретные ключи (MAC/SKI/DEA от ВТБ) нужны только для онлайн-авторизации, НЕ для чтения PAN**; для доведения `startEMV` до PAN нужны только публичные терминальные параметры (AID-table/CAPK). Диагностика: `app-payment` `core/BankCardProbe.kt`+`EmvProbe.kt`, endpoints `GET /probe/card`, `/probe/emv`, `/probe/startemv` (loopback 8790). `PayProcessor` (`PO_C_MOCK_EMV`) — корректный заменитель до имплементации EMV-флоу + загрузки параметров эквайером.
