...
./gradlew :android-terminal:app:clean :android-terminal:app:assembleDebug  # Android — допустимо (только свой модуль, никаких shared deps в kotlin); селекторы вложенных билдов — по имени директории, НЕ по пути (:android-terminal, не :frontend:android-terminal)
```
...
- **NFC:** 13.56 МГц, ISO/IEC 14443 (Type A&B), ISO 18092, Felica, Mifare — поддерживает DESFire EV1/EV2/EV3 (ISO 14443-4). **EMV-доступ ПОДТВЕРЖДЁН РАБОЧИМ (2026-09-21):** лицензионное ядро FTSDK `Emv` доступно стороннему приложению — `Emv.searchCard(2)` (cardType=2=`CARD_TYPE_RF`) детектит МИР-карту с обменом ATS; `Emv.startEMV` стартует (`startTransaction`, TransData задаётся). **PAN карта отдаёт только внутри полной EMV-транзакции** (`SELECT→GPO→READ RECORD` в `startEMV`), а НЕ через `getCardData("5A")`/`GET DATA` вне транзакции (отвечает «данные недоступны»). Сырой `NfcReader.openCardEx(0,…)` даёт `error=89` (`ERR_OP_TIMEOUT`) — там тоже нужен корректный cardType=2/RF, а не 0/IC. **Секретные ключи (MAC/SKI/DEA от ВТБ) нужны только для онлайн-авторизации, НЕ для чтения PAN**. **Полный флоу `startEMV` доведён (порт `EmptyEventHandler` из легаси `feitian-hardware`):** ядро — интерактивная стейт-машина, приложение ОБЯЗАНО отвечать на колбэки (`respondEvent`/`setTLV("1F3E"/"1F10"/"1F6A")`/`setIssuerOnlineResponseData`), иначе флоу виснет на `cbWaitCard`. Передав полный `TransRequest` (`setCardType(2)`, `setReadRecordCallback(true)`, `setEnableAppSelectCallback(false)`, `setAdditionalTlvData("1F300101")`, `setMaxTimeoutEMVThreadWait(30)`) + `onSearchCard`→`searchCard(N, cb)` → карта найдена (`[found:2]`), preprocessing и выбор приложения идут. **Блокер до PAN через ядро — пустая AID-таблица** (`Valid AID Num: 0` → «Contactless Application Not Allowed», `cbEndProcess ErrCode 17/23`): нужна загрузка `EMVCL_AppParameters` от эквайера (`manageEmvclAppParameters(ADD, tlv)` — МИР/RUPAY AID + capabilities + TAC, в логе `RupayICS=07582F`). **Причина rc=5 (2026-09-22)**: `manageEmvclAppParameters(ADD)` = `rc=5` «System internal error» **равномерно для МИР/RUPAY/Mastercard/Visa** (`CMirBean`/`CRupayBean`/`CMastercardBean`/`CVisaBean` — не формат и не карта), это гейт `com.ft.apipermission.provider` (`ApiPermissionCheckUtil.checkApiPermission`, `CERT_TYPE`, `isMiuraSubCert`): инъекция конфига эквайера — только аккредитованному приложению (`setAddAppWhitelist` не помогает, `addAppSignCertificate`→false). **МИР обслуживает ядро RUPAY** (ядра MIR нет; KernelID МИР=`0D`; схемный бин — `CRupayBean`, НЕ `CEmvAidBean`/`CMirBean`), терминальные дефолты — `setDefaultAppParameters` (`DefaultParamsBean`) до ADD. **Обход без ядра (2026-09-22, РАБОТАЕТ)**: публичные данные карты читаются сырыми APDU через `NfcReader.openCardEx(2)` + `sendApduCustomer` (SELECT PPSE → SELECT AID → GPO → READ RECORD): PAN/masked PAN/срок/держатель/label/track2. GPO МИР требует **values-only PDOL** (`83 <len> <значения БЕЗ tag/len>`, иначе `6700`/`6985`). Реализация `EmvCardReader.kt` (+`EmvclParamsLoader`/`AsopApiPermission`), endpoints `GET /probe/card`, `/probe/emv`, `/probe/startemv`, `/probe/emvcard`, `/probe/kernel/{id}`, `/probe/whitelist`, `/probe/signcert`, `/probe/emvfiles` (loopback 8790). `PayProcessor` (`PO_C_MOCK_EMV`) теперь читает реальную карту (`EmvCardReader.read`) и отдаёт фактический `CardInfo` (maskedPan/panLast4/bin/expiry/holdername); фолбэк — заглушка.
...
- **Android**: `TerminalKeyEntity` (KEY_ID PK, KEY_MATERIAL_ENC, CREATED_AT, DELETED_AT, VERSION) в `AppDatabase` **v5**. В `ReferenceSyncStore.applyChunk/applyFile` ветка `tableName == "asop_keys"` → сохраняется в `terminal_keys`, перешифрованный локальным Keystore-AES-ключом (`TerminalKeyCryptor`, AndroidKeyStore AES-GCM, PURPOSE_ENCRYPT|DECRYPT, неэкспортируемый). НЕ в `reference_rows` (иначе ломается watermark `maxVersion()`). Записи c DELETED_AT физически удаляются. Порядок KEY_ID DESC (UUIDv7, свежие первыми). Для MIFARE Classic: первые 6 байт = Key A, байты 6-11 = Key B.
  ...
  **Детали `TerminalKeyCryptor`:**
  ...
  - **Живёт в общей lib `asop-nfc-lib` (`frontend/android-nfc`, composite build)**: терминал подключает как `implementation("ru.asop.nfc:asop-nfc-lib:0.1.0")`. `android-nfc` включается на уровне ROOT (sibling composite, рядом с `frontend/android-terminal`) — НЕ через `includeBuild("../android-nfc")` в terminal/distributor: вложенный includeBuild внутри включённого билда ломает типобезопасные accessors Gradle 8.10 (`RootProjectAccessor.getAsopNfcLib()` defined twice). Сборка Android — из корня репо. Биндится через Hilt-модуль `AppModule.provideTerminalKeyCryptor()` (`@Provides @Singleton`) — в lib класс без `@Inject`. См. «Общая NFC/VCM1-библиотека» в разделе Frontend.
...
- **`frontend/web-admin/`**: Vite + React + TypeScript + react-router + TanStack Query + oidc-client-ts
  ...
  **Общая NFC/VCM1-библиотека** (`frontend/android-nfc/asop-nfc-lib`, group `ru.asop.nfc`, v0.1.0, composite build) — общие классы NFC/VCM1 для `android-terminal` и `android-distributor`: `AsopCardType` (+`CardActivationMatrix`, `allRolesForBitmask`, `highestSetBitRole`, `bitmaskForEntityRoles`), `EntityType`/`EntityRef` (clean-break: только USER/NONE), `CardIdentityVcm1` (VCM1 encode/decode, magic, tripsLeft), `Vcm1CardAuth` (`read`/`readWithSession` + `Session.updateTrips` в той же mfc-сессии — F20 требование), `MifareClassicVcm1` (detectState/write/writeTripsLeft, multiPassWrite, factory-keys), `TerminalKeyCryptor` (AES-GCM Keystore, без Hilt-аннотаций). Подключение: `implementation("ru.asop.nfc:asop-nfc-lib:0.1.0")`; композит `android-nfc` включён на уровне корневого `settings.gradle.kts` (`includeBuild("frontend/android-nfc")`, sibling с терминалом) — см. «Ключи ASOP_KEYS» выше. **Миграция терминала (2026-09-19)**: дубли `ru.asop.terminal.activation.{AsopCardType,EntityType,CardIdentityVcm1}` + `nfc/Vcm1CardAuth` + `db/TerminalKeyCryptor` удалены, usage переключен на `ru.asop.nfc.*`; `TerminalKeyCryptor` биндится через `AppModule.provideTerminalKeyCryptor()` (`@Provides @Singleton`, в lib без `@Inject`). **Осталось в терминале** (терминал-специфичное): `nfc/MifareClassicCardWriter` (SAC1-легаси + `readVcm1` + `DetectResult`/matchedKey), `nfc/MifareClassicReader`, `nfc/DesfireCardReader`, `cardIdentity` UI/VMs. Тесты VCM1 (`Vcm1CardAuthTest`, `CardIdentityVcm1Test`) — зелёные.
...
- **`frontend/android-distributor/`** (standalone Gradle-сборка, `namespace ru.asop.distributor`, `includeBuild("../android-nfc")`, БЕЗ Hilt — ручной DI через `AppGraph`): приложение дистрибьютора карт (выдача/пополнение VCM1), sibling-композит `android-nfc`. Серверный контур — **прямые mTLS `/api/v1/sync/distributor/**`** (НЕ orchestrator delta, НЕ proto):
  - `POST /api/v1/sync/distributor/register` → terminal-service `POST /api/v1/distributor-terminals/register` — идемпотентный upsert `ASOP_DISTRIBUTOR_TERMINALS` по `TERMINAL_SERIAL` (201 create / 200 update / 400 новый serial без `cardsDistributorId`+`paymentProviderId`; `terminalNumber` default = serial).
  - `GET /api/v1/sync/distributor/keys/delta` → admin `GET /api/v1/asop-keys/delta`; `GET /api/v1/sync/distributor/tariffs/delta` → card `GET /api/v1/tariff-rates/delta` — JSON-keyset pull (тот же `versionSince`, что у оркестратора). Ошибка мастера → 502.
  - `asop_keys`/`asop_tariff_rates` — GLOBAL_TABLES, distributor тянет целиком (scope-фильтрация не нужна). `ASOP_DISTRIBUTOR_TERMINALS` не delta: нет VERSION/DELETED_AT.
  - **Идентичность — отдельный distributor cert-sign** (2026-09-20): `POST /api/v1/distributor-terminals/cert-sign` (open HTTPS + HMAC `X-API-Key`/`X-Timestamp`/`X-Signature`, `CertSignHmacFilter` допускает оба пути `…/terminals/cert-sign` и `…/distributor-terminals/cert-sign`; `SecurityConfig` Order(3) permitAll для POST). `CertSignRequest`/`CertSignRequested`/`CertIssued` получили флаги `distributor`/`cardsDistributorId`/`paymentProviderId` (греют дверь и в `render-модуль`). Сага: gateway `CertCommandService.publishDistributor` → Kafka `asop.terminal.cert.commands` → crypto (>distributor passthrough) → `asop.terminal.cert.issued` → terminal-service `CertCommandService.handleDistributor` upsert'тит `ASOP_DISTRIBUTOR_TERMINALS` (НЕ `ASOP_TERMINALS`, НЕ `ASOP_TERMINAL_CERTS`) и эмитит `CertStored` с `certId = UuidUtils.newId()` (сервер-side сертификат не хранится). **ВАЖНО**: `uq_distributor_terminals_provider_id` (UNIQUE `PAYMENT_PROVIDER_ID`) — на каждую строку нужен уникальный provider; `findByTerminalSerial` upsert'ит по серии — одинаковый `ANDROID_ID` (тот же sign-ключ) сливает строки, поэтому разные приложения приставами к serial (см. app-payment ниже).
  - `SyncStore` (DataStore: serial, distributor_terminal_id, last_keys_version, last_tariffs_version, keys_json, tariffs_json, synced_at_ms), `ProvisionSyncManager` (certify → poll 4-hop → register → delta → merge). `KeyProvider`/`TariffProvider` переписаны на `sync_store` (defaultFare = min(price) live-записи с carrierId/zoneId/pathId = null → fallback 30.0). **Гэп (dev OK, prod TODO)**: прямой `keys/delta` отдаёт `KEY_MATERIAL` = RSA-шифротекст (базиться публичным ключом сервера), а `KeyProvider` пытается AES-GCM-дешифровать через `TerminalKeyCryptor` → `decrypt key … failed` и `candidateKeys()` откатывается на `DEV_KEY(24) + ZERO_KEY(6)`. В dev-режиме `DEV_KEY` (`AAECAwQFBgc…` = 00..17) совпадает с dev-ключом crypto, поэтому тап работает; для prod нужен RSA→AES-GCM-трансформ (как `KeyService` оркестратора) до `mergeKeys`.
  - BuildConfig: `GATEWAY_BASE_URL` (`gateway.host` из `local.properties`, default `10.0.2.2`), `CERT_SIGN_API_KEY`/`CERT_SIGN_HMAC_SECRET` (dev-дефолты = gateway `asop.cert-sign-api-keys`), `DISTRIBUTOR_CARDS_DISTRIBUTOR_ID` (default сид Крыма `00000000-0000-0000-0000-112000000300`), `PAYMENT_PROVIDER_ID` (default `MOCK`). Moshi — рефлексия (`KotlinJsonAdapterFactory`), без kapt/codegen.
  - Отдельный док: `doc/payment-plan.md` → «Статус Фазы 5».

- **`frontend/android-payment/`** (`ru.asop.payment`, app-payment — банковская плата на F20, отдельный APK): свои `cert/CertManager` (алиас `asop_payment_cert`), `cert/AsopKeyManager`, `network/CertSignHmacInterceptor`, `network/ProvisioningManager`, `network/AcquirerReportClient`. **mTLS переиспользует distributor cert-sign** (крёст решение): `POST /api/v1/distributor-terminals/cert-sign` с `distributor=true` + свой `PAYMENT_PROVIDER_ID=MOCK-PAY` и **префикс serial `PAY-<ANDROID_ID>`** (чтобы `findByTerminalSerial` не слил в одну строку с distributor — тот же `ANDROID_ID` из-за общего debug-ключа) → вторая строка `ASOP_DISTRIBUTOR_TERMINALS`. После COMPLETED цепочка PEM кладётся в `CertManager`, ставится `DistributionConfig.setPaired(true)`. `AcquirerReportClient` постит `POST /api/v1/payment/report` по клиентскому сертификату (OkHttp + `AsopKeyManager`, trust-all MVP); до pairing — `DEFERRED` (store-and-forward). Запуск provisioning — `PaymentApp.onCreate` (бэкграунд-поток). BuildConfig: `CERT_SIGN_API_KEY`/`CERT_SIGN_HMAC_SECRET`/`DISTRIBUTOR_CARDS_DISTRIBUTOR_ID`/`PAYMENT_PROVIDER_ID`(default `MOCK-PAY`); OkHttp добавлен в deps (раньше не было Retrofit/OkHttp — только org.json).
