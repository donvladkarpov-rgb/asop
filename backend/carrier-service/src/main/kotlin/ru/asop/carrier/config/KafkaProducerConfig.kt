package ru.asop.carrier.config
...
class KafkaProducerConfig {
    ...
    fun reactiveKafkaProducerTemplate(): ReactiveKafkaProducerTemplate<String, Any> {
        val props = HashMap<String, Any>().apply {
            ...
            put(ProducerConfig.CLIENT_ID_CONFIG, "carrier-service-producer")
            put(JsonSerializer.ADD_TYPE_INFO_HEADERS, false)
            put("security.protocol", "SSL")
            put("ssl.truststore.location", trustStorePath.removePrefix("file:"))
            put("ssl.truststore.password", trustStorePassword)
            if (keyStorePath.isNotBlank()) {
                put("ssl.keystore.location", keyStorePath.removePrefix("file:"))
