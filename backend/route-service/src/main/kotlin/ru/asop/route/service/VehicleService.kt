...
import ru.asop.route.entity.VehicleEntity
import ru.asop.route.repository.GenericRouteRepository
import ru.asop.route.repository.RouteTableRegistry
import java.util.UUID

@Service
class VehicleService(private val repo: GenericRouteRepository) {

    private val tableInfo = RouteTableRegistry.resolve("vehicles")
        ?: error("vehicles not found in registry")
    ...
    fun create(data: Map<String, String?>): Mono<Map<String, Any?>> {
        val entity = VehicleEntity.fromRequest(data)
        return repo.create(tableInfo, entity.vehicleId, entity.toDbMap().filterKeys { it.lowercase() != tableInfo.pkColumn.lowercase() })
    }

    fun update(id: String, data: Map<String, String?>): Mono<Map<String, Any?>> {
        val entity = VehicleEntity.fromRequest(data + ("id" to id))
