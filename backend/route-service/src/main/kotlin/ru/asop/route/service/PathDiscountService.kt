...
import ru.asop.route.entity.PathDiscountEntity
import ru.asop.route.repository.GenericRouteRepository
import ru.asop.route.repository.RouteTableRegistry
import java.util.UUID

@Service
class PathDiscountService(private val repo: GenericRouteRepository) {

    private val tableInfo = RouteTableRegistry.resolve("path-discounts")
        ?: error("path-discounts not found in registry")
    ...
    fun create(data: Map<String, String?>): Mono<Map<String, Any?>> {
        ...
        return repo.create(tableInfo, entity.pathDiscountId, entity.toDbMap().filterKeys { it.lowercase() != tableInfo.pkColumn.lowercase() })
    }
