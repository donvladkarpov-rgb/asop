...
class ScheduleService(private val repo: GenericRouteRepository) {
    ...
    fun create(data: Map<String, String?>): Mono<Map<String, Any?>> {
        val entity = ScheduleEntity.fromRequest(data)
        return repo.create(tableInfo, entity.scheduleId, entity.toDbMap().filterKeys { it.lowercase() != tableInfo.pkColumn.lowercase() })
    }

    fun update(id: String, data: Map<String, String?>): Mono<Map<String, Any?>> {
        val entity = ScheduleEntity.fromRequest(data + ("id" to id))
