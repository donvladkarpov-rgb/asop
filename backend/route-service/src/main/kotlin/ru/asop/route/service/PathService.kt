...
import ru.asop.route.entity.PathEntity
import ru.asop.route.repository.GenericRouteRepository
import ru.asop.route.repository.RouteTableRegistry
import java.util.UUID

@Service
class PathService(private val repo: GenericRouteRepository) {

    private val tableInfo = RouteTableRegistry.resolve("paths")
        ?: error("paths not found in registry")
    ...
    fun create(data: Map<String, String?>): Mono<Map<String, Any?>> {
        val entity = PathEntity.fromRequest(data)
        return repo.create(tableInfo, entity.pathId, entity.toDbMap().filterKeys { it.lowercase() != tableInfo.pkColumn.lowercase() })
    }

    fun update(id: String, data: Map<String, String?>): Mono<Map<String, Any?>> {
        val uuid = parseId(id)
        val entity = PathEntity.fromRequest(data + ("id" to id))
