...
class ExceptionHandler {
    ...
    fun handleDataIntegrityViolation(e: DataIntegrityViolationException): ResponseEntity<String> {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.mostSpecificCause.message)
    }

    @ExceptionHandler(ResponseStatusException::class)
    fun handleResponseStatus(e: ResponseStatusException): ResponseEntity<String> {
        log.warn("Status {}: {}", e.statusCode, e.reason)
        return ResponseEntity.status(e.statusCode ?: HttpStatus.INTERNAL_SERVER_ERROR).body(e.reason)
    }
}