...
- [x] **VERSION cursor** — глобальный sequence `asop_delta_version_seq` + `trg_fn_assign_version()` (INSERT/UPDATE/DELETE → `VERSION = nextval(...)`). Все 42 справочные таблицы имеют `VERSION BIGINT`. **Дельта-курсор — VERSION, а не UPDATED_AT** (монотонный глобальный sequence, не зависит от таймзоны).
