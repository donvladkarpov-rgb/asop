...
- До PAN: полный флоу `startEMV` доведён (порт `EmptyEventHandler` из легаси `feitian-hardware`: `respondEvent`/`setTLV`/`setIssuerOnlineResponseData` + полный `TransRequest` + `onSearchCard`→`searchCard`) — карта найдена (`[found:2]`), preprocessing/выбор приложения идут. **Блокер — пустая AID-таблица** (`Valid AID Num: 0` → «Contactless Application Not Allowed», `cbEndProcess 17/23`): нужны `EMVCL_AppParameters` эквайера (`manageEmvclAppParameters`, МИР/RUPAY AID + capabilities + TAC). Минимальный `CEmvAidBean(aid).toTlvByteArray()` невалиден (`rc=5`). Секретные MAC/SKI/DEA (ВТБ) — только для онлайн-авторизации.
