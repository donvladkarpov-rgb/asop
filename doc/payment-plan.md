...
- До PAN: полный флоу `startEMV` доведён (порт `EmptyEventHandler` из легаси `feitian-hardware`: `respondEvent`/`setTLV`/`setIssuerOnlineResponseData` + полный `TransRequest` + `onSearchCard`→`searchCard`) — карта найдена (`[found:2]`), preprocessing/выбор приложения идут. **Блокер — пустая AID-таблица** (`Valid AID Num: 0` → «Contactless Application Not Allowed», `cbEndProcess 17/23`): нужны `EMVCL_AppParameters` эквайера (`manageEmvclAppParameters`, МИР/RUPAY AID + capabilities + TAC). Секретные MAC/SKI/DEA (ВТБ) — только для онлайн-авторизации.
- **Причина rc=5 (2026-09-22)**: `manageEmvclAppParameters(ADD, …)` возвращает `rc=5` = «System internal error» **равномерно для МИР/RUPAY/Mastercard/Visa** (проверено `CMirBean`/`CRupayBean`/`CMastercardBean`/`CVisaBean`) — это не формат TLV и не карта, а гейт `com.ft.apipermission.provider` (`ApiPermissionCheckUtil.checkApiPermission`, `CERT_TYPE`, `isMiuraSubCert`): инъекция терминального конфига эквайера доступна только аккредитованному приложению. `setAddAppWhitelist` (power-save) не помогает, `addAppSignCertificate` → `false`. **Обход: ручное чтение публичных данных карты по APDU, без ядра — см. ниже.**
- Диагностика в `app-payment`: `core/BankCardProbe.kt`, `core/EmvProbe.kt`, endpoints `GET /probe/card`, `/probe/emv`, `/probe/startemv`, `/probe/kernel/{id}`, `/probe/whitelist`, `/probe/signcert`, `/probe/emvfiles`, `/probe/emvcard`.

## Статус 2026-09-22 — ручное чтение банковской карты по APDU (`EmvCardReader`), без EMV-ядра

`manageEmvclAppParameters(ADD)` заблокирован гейтом FTSDK (см. выше), но **публичные данные карты читаются напрямую сырыми APDU** через `NfcReader` (подход devnied/EMV-NFC-Paycard-Enrollment) — ядро и `EMVCL_AppParameters` для этого НЕ нужны.

**Работает на F20 (карта МИР `A0000006581010`):**
- `NfcReader.openCardEx(2 /* CARD_TYPE_RF */, …)` → карта открыта (НЕ `0`/IC — тот даёт `89`).
- `sendApduCustomer(...)` — ручной ISO-DEP: **SELECT PPSE** (`00 A4 04 00 0E 325041592E5359532E4444463031 00`) → `61…4F…` (AID МИР); **SELECT AID** → FCI с `50`(label `MIR`), `9F38`(PDOL); **GPO** → `94`(AFL); **READ RECORD** → `5A`(PAN), `57`(track2), `5F24`(exp), `5F20`(holder).
- **Ключ (иначе `6700`/`6985`)**: данные GPO для МИР — `83 <len> <PDOL-значения БЕЗ tag/length>` (values-only), а не стандартный `83 <len> <tag+len+value>`. PDOL МИР = `9F7A01 5F2A02 9F0206 9F3501 9F4005`; дефолты в `PDOL_DEFAULTS` (`9F7A=01`, `5F2A=0643`, `9F02=000000000000`, `9F35=22`, `9F40=FF80F0A001`). `tryGpo` перебирает `full`/`empty`/`values-no83`/`values-83`, берёт первый с успехом.
- Итог: `{connected, pan, maskedPan:XXXXXX******YYYY, cardholderName, expDate:MM/YY, appLabel, aid, track2}`. `maskPan` = first6 + `******` + last4.
