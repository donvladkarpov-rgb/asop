...
4) На терминале при регистрации пользователь вводит регион, перевозчика и таймзону. Необходимо их сохранить в базе терминала и передавать во **всех** запросах к серверу. Ресолв региона и перевозчика из gateway убрать. В `doc/architecture.md`, `doc/context.md` и `AGENTS.md` зафиксировать, что gateway только для авторизации, аутентификации и проксирования, бизнес-логики там нет и быть не может.
   ...
   - **Регистрация**: `TerminalRegisterRequest` в **трёх местах** — Android (`TerminalModels.kt`), gateway-api (`backend/shared/api/gateway-api/.../dto/request/TerminalRegisterRequest.kt`), terminal-api (`backend/shared/api/terminal-api/.../dto/request/TerminalRegisterRequest.kt`). В terminal-api и Android добавить `regionId` (nullable; `timezone` уже есть, `carrierId` уже есть). **gateway-api `TerminalRegisterRequest` — мёртвый код** (нигде не импортируется, `ProxyController` не десериализует тело) — **удалить файл целиком** (`backend/shared/api/gateway-api/.../request/TerminalRegisterRequest.kt`). Регистрацию валидирует terminal-api DTO, gateway проксирует сырое тело.
   - **Сохранение на терминале**: `SyncPreferences` хранит `regionId`, `carrierId`, `timezone` (новые ключи + геттеры/сеттеры). Заполняются при регистрации (из `RegistrationScreen`, где пользователь уже вводит регион/перевозчика/таймзону). Таймзона определяется автоматически (`TimeZone.getDefault().id`), пользователь может изменить через dropdown.
   - **Delta/full** (`/api/v1/sync/references/delta`, `/full`): `DeltaSyncRequest`/`FullSyncRequest` получают `carrierId`, `regionId`. Терминал берёт их из `SyncPreferences` (это уже заложено в 003_02, этапы 5–6 — согласовать, не дублировать; при этом `DeltaSyncWorker`, `FullDumpDownloadWorker`, `ReferenceSyncViewModel.fullSync` явно читают `carrierId.first()`/`regionId.first()` из SyncPreferences и передают в запрос). Gateway **перестаёт** вызывать `TerminalResolver` — берёт `carrierId/regionId` прямо из запроса (сделано в 003_02).
   - **Все 10 sync-командных эндпоинтов** (`/api/v1/sync/sessions/open`, `/close`, `/transactions`, `/cards/register`, `/cards/{id}/block`, `/debts`, `/debts/{id}/recover`, `/fiscal/receipts`, `/audit/tasks`, `/gps/positions`): в request-DTO добавить `regionId` и `timezone` (nullable) **везде**; `carrierId` — только где его ещё нет (в `DebtCreateRequest`/`AuditTaskCreateRequest` уже есть). Модули DTO: session-api (`SessionOpenRequest`/`SessionCloseRequest`), card-api (`CardRegisterRequest`/`CardBlockRequest`), debt-api (`DebtCreateRequest`/`DebtRecoverRequest`), fiscal-api (`FiscalReceiptRequest`), audit-api (`AuditTaskCreateRequest`), gateway-api (`TransactionCompleteRequest`/`GpsPositionReport`). Android-модели (`SyncModels.kt` и др.) — соответствующие поля. В Kafka-события поля НЕ добавлять — gateway кладёт контекст в Kafka headers (`X-Carrier-Id`/`X-Region-Id`/`X-Timezone`, по аналогии с `X-Event-Id`/`X-Keycloak-Id`); консьюмеры могут читать или игнорировать.
...
* [`ASOP_CARDS_DISTRIBUTORS`](#asop_cards_distributors) [carrier-service]
* [`ASOP_CONTRACT_ROUTES`](#asop_contract_routes) [route-service]
* [`ASOP_VEHICLE_TYPES`](#asop_vehicle_types) [route-service]
* [`ASOP_VEHICLE_MODELS`](#asop_vehicle_models) [route-service]
* [`ASOP_VEHICLES`](#asop_vehicles) [route-service]
...
2) Напиши в папке `frontend` ещё одно Android-приложение для тестовых целей, назови `android-test`, сделай там меню с тремя пунктами:
   ...
   **Процесс тестирования:**
   ...
   - Накатываем `seed-data-delta-1.sql` → в **android-terminal** запускаем дельта-синк: drawer → «Загрузить справочники» → **«Дельта сейчас»** → дожидаемся **COMPLETED** в карточке синхронизации → в **android-test** нажимаем **"Тест дельта инкремента 1"** → проверяем, что в БД android-terminal (через `ContentProvider`) присутствуют отфильтрованные данные ровно как в `expected-1.json` (`seed-data.sql` **и** `seed-data-delta-1.sql`).
   - Накатываем `seed-data-delta-2.sql` → снова «Дельта сейчас» в android-terminal → COMPLETED → в android-test **"Тест дельта инкремента 2"** → проверяем отфильтрованные данные как в `expected-2.json` (`seed-data.sql` + delta-1 + delta-2).
   - Накатываем `seed-data-delta-3.sql` → в **android-terminal** (сам терминал) запускаем полную выкачку через S3 (кнопка «Полная выкачка» / `enqueueFullDump`) → дожидаемся скачивания ZIP → в **android-test** нажимаем **"Получить все данные"** → проверяем данные как в `expected-all.json` (полный итог: `seed-data.sql` + delta-1 + delta-2 + delta-3).
   - Данные проверяются через `ContentProvider` внутри android-terminal (query API из п.3).
   - **ВАЖНО:** android-test **не может** сам инициировать mTLS-синк (сертификат и `SyncApi` — в android-terminal). Синк всегда запускается вручную в android-terminal («Дельта сейчас» / «Полная выкачка»), кнопки android-test только проверяют результат.
   - **Эталон JSON:** `created_at`/`updated_at` из сравнения исключить (при bulk-INSERT через `generate_series` триггер ставит `now()` — недетерминировано). Сравнивать `version` (точное), `deleted_at` (null для активных), все business-поля.
