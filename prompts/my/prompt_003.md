...
1) Для поля UPDATED_AT в таблицах/справочниках сделать триггер, котороый автоматом заполняет поле датой при апдейтах и
инсертах. Значение прописанное в инсерте или апдейте игнорировать.
2) Сделать ContentProvider внутри android-terminal (с query API) для всех таблиц, которые есть в его базе
3) На терминале при регистрации пользователь вводит регион и перевозчика и тайм зону, необходиом их сохранить в базе и
...
4) выполни промпт prompts/my/prompt_003_01.md
...
"Тест дельта инкремента 1" проверять в базе данных приложения android-terminal, что все данные из  
infrastructure/docker/seed-data-delta-1.sql получены терминалом.
По нажатии на меню "Тест дельта инкремента 2" проверять в базе данных приложения android-terminal, что все данные из  
infrastructure/docker/seed-data-delta-1.sql и infrastructure/docker/seed-data-delta-2.sql получены терминалом.
По нажатии на меню "Тест выкачки всего сразу" проверять в базе данных приложения android-terminal, что все данные из  
